...
namespace QuantityMeasurementService.Repositories;
...
public class QuantityMeasurementRepository : IQuantityMeasurementRepository
{
    ...
    {
        var entities = _context.QuantityMeasurements.Where(m => m.UserId == userId).ToList();
        if (entities.Any())
        {
            _context.QuantityMeasurements.RemoveRange(entities);
            _context.SaveChanges();
            _logger.LogInformation("Deleted all measurements for user {UserId}", userId);
        }
    }
}
